...
def viral_score(df):
    ...
    df["view_count_normalized"] = (df["view_count"] - min_values["view_count"]) / (
        max_values["view_count"] - min_values["view_count"]
    )
    df["engagement_rate_normalized"] = (
        df["engagement_rate"] - min_values["engagement_rate"]
    ) / (max_values["engagement_rate"] - min_values["engagement_rate"])
    df["average_daily_views_normalized"] = (
        df["average_daily_views"] - min_values["average_daily_views"]
    ) / (max_values["average_daily_views"] - min_values["average_daily_views"])
    df["viral_score"] = (
        0.5 * df["view_count_normalized"]
        + 0.2 * df["engagement_rate_normalized"]
        + 0.3 * df["average_daily_views_normalized"]
    ).round(2)
    return df
...
def preprocess_dataframe(df, key):
    ...
    df[["duration", "subscriber_count", "view_count", "comment_count"]] = df[
        ["duration", "subscriber_count", "view_count", "comment_count"]
    ].astype(int)
    ...
    df["engagement_rate"] = (
        (df["likes"] + df["comment_count"]) / df["view_count"]
    ).round(2)
    ...
    df["time_to_trend"] = (
        df["trending_date"].dt.tz_localize(None)
        - df["publishedAt"].dt.tz_localize(None)
    ).dt.days
    ...
    df["average_daily_views"] = (
        df.apply(
            lambda x: (x["view_count"] / 1)
            if x["time_to_trend"] == 0
            else (x["view_count"] / x["time_to_trend"]),
            axis=1,
        )
        .round()
        .astype(int)
